...
class Validator:
    ...
    EMAIL = {'from_': {'command': 'isemail', 'type': 'email address'},
             'to': {'command': 'isemail', 'type': 'email address'},
             'cc': {'command': 'isemail', 'type': 'email address or list of email addresses'},
             'bcc': {'command': 'isemail', 'type': 'email address or list of email addresses'},
             }

    TWILIO = {'from_': {'command': 'isphone', 'type': 'phone number'},
              'to': {'command': 'isphone', 'type': 'phone number'},
              'media_url': {'command': 'isurl', 'type': 'url'},
              }

    SLACKWEBHOOK = {'webhook_url': {'command': 'isurl', 'type': 'url'},
                    'attach_urls': {'command': 'isurl', 'type': 'url or list of urls'},
                    }
    ...
    def _is_valid(msgtype, attr, val):
        ...
        command = getattr(validus, msgtype[attr]['command'])
        return command(val)
